...
fn main(mut req: Request) -> Result<Response, Error> {
    ...
    match (
        req_clone_for_match.get_method(),
        req_clone_for_match.get_path(),
    ) {
        ...
        (&Method::GET, url_path) if url_path.ends_with(".m3u8") => {
            ...
            let path_segments = url.path_segments().ok_or_else(|| "cannot be base").unwrap();
            ...
            let base_url = req_clone.get_url().host_str().unwrap();
            let mut path_url_vec = vec![];
            path_url_vec.push(base_url);
            for (_i, path) in path_segments
                .enumerate()
                .filter(|&(index, _path)| index < len - 1)
            {
            ...
            if be_resp.get_status() == StatusCode::OK {
                ...
                match m3u8_rs::parse_playlist_res(&be_resp_body) {
                    ...
                    Ok(Playlist::MediaPlaylist(pl)) => {
                        ...
                        for (_index, segment) in m_playlist
                            .segments
                            .iter()
                            .enumerate()
                            .filter(|&(index, _)| index < 6)
                        {
                            ...
                            println!(
                                "Prefetching this segment in call to playlist {}",
                                format!("{}{}", base_path, segment.uri)
                            );
                            //let segment_uri = base_url + segment.uri.;
                            let pending_req_val = fastly::Request::head(format!(
                                "https://{}{}",
                                base_path, segment.uri
                            ))
                        ...
                        let _resp_bytes = playlist_resp.write_to(&mut resp_vec).unwrap();
                        // We respond with the new playlist
                        Ok(Response::new().with_body(Body::from(resp_vec)))
                    }
                    Err(_e) => Ok(be_resp),
                }
            } else {
        ...
        (&Method::GET, url_path) if url_path.ends_with(".ts") => {
            ...
            req.set_url(
                Url::parse(
                    format!(
                        "https://{}{}",
                        base_path,
                        req_clone.get_query_str().unwrap()
                    )
                    .as_str(),
                )
                .unwrap(),
            ...
            if be_resp.get_status() == StatusCode::OK {
                ...
                match m3u8_rs::parse_playlist_res(&be_resp_body) {
                    ...
                    Ok(Playlist::MediaPlaylist(pl)) => {
                        let m_playlist = pl.clone();
                        for (index, segment) in m_playlist
                            .segments
                            .iter()
                            .enumerate()
                            //.filter(|&(index, _)| index < 5)
                            .filter(|&(index, segment)| {
                                (index % 5 == 0) && index > 0 && segment.uri == file
                            })
                        {
                            println!(
                                "This is the segment uri {}, index {} and this is the file name {}",
                                segment.uri, index, file
                            );
                            let mut pending_req_vec = vec![];
                            for (_index, segment) in
                                m_playlist
                                    .segments
                                    .iter()
                                    .enumerate()
                                    .filter(|&(new_index, _)| {
                                        new_index > index && new_index < index + 6
                                    })
                            {
                                // You will go through every item in playlist one by one in the order you requested
                                // Forward the downstream request to the backend
                                println!(
                                    "Prefetching this segment in call to ts {}",
                                    format!("{}{}", base_path, segment.uri)
                                );
                                //let segment_uri = base_url + segment.uri.;
                                let pending_req_val = fastly::Request::head(format!(
                        ...
                        println!(
                            ...
                            format!("https://{}{}", base_path, file)
                        );
                        let be_mut_beresp =
                            fastly::Request::get(format!("https://{}{}", base_path, file))
                                .send(PRIMARY_BACKEND)?;
                        Ok(be_mut_beresp)
                    }
                    Err(_e) => Ok(be_resp),
                }
